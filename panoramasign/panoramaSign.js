 ...
 */
 ...
 const PORTS = [10029, 10030, 10031, 10032], HOST = '93.89.113.12';
 const STX = 0x02, ETX = 0x04, ACK = 0x06, NACK = 0x15, CRC = 0x00;
         ...
         client.on('data', function(data) {
             try {
                 if(data[1].toString(16) == ACK){
                     ...
                     client.end();
                     reject("NACK")
                 }else if(data[0].toString(16) == ACK) { // TODO: Når avalanche oppdateres svarer tavle med <Buffer CTX>, og så ny melding med <Buffer ACK, CMD....>. Finne ut hvorfor
                     client.end();
                     resolve();
                 }
             } catch(error) {
 ...
 module.exports = {updatePanoramaSign, avalancheTelegramConstructor, relaysTelegramConstructor, setAllRelaysTelegramConstructor, clearDisplayTelegramConstructor}